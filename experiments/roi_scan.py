...
if __name__ == "__main__":
    ...
    while 1:
        ...
        scan_win = cvRect(x*320,y*240,320,240)
        #print str(scan_win)
        # img0 = cvGetSubRect( img, None, scan_win)
        img0 = cvCloneImage( img )

        

        cvSetImageROI(img0, scan_win)

        cvResetImageROI(img0)

        cvRectangle( img0, CvPoint(scan_win.x,scan_win.y), CvPoint(scan_win.x+scan_win.width, scan_win.y+scan_win.height), CV_RGB(255,255,0), 15, CV_AA, 0 )
