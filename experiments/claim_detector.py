            ...
            if has_code and not self.decoded[window_name]:
                ...
                if data != "NO BARCODE":
                    ...
                    if match:
                        ...
                        if user_card:
                            self.zonecolor[window_name] = CV_RGB(255,0,0)
                            # draw.text((int(self.display_scale*rect.x), int(self.display_scale * rect.y-40)), "Card already claimed.", font=data_font, fill=(0,200,0,0))
                            print '''%s decoded %s''' % (window_name,user_card['full_name'])
                            self.lock.acquire()
                            self.visible_user[window_name] = user_card['user_id']
                            self.lock.release()
                        else:
                            webbrowser.open('http://snapmyinfo.com/claim/request/%s' % (code))
...
if __name__ == "__main__":
    ...
    disp_left_x = int(0.5 * width - int(display_dim / 2.0))
    disp_right_x = int(0.5 * width - int(display_dim / 2.0))
    disp_both_y = int(0.5 * height - int(display_dim / 2.0))

    scan_dim = int(scan_size * o_height)
    left_x = int(0.5 * o_width - int(scan_dim / 2.0))
    right_x = int(0.5 * o_width - int(scan_dim / 2.0))
    both_y = int(0.5 * o_height - int(scan_dim / 2.0))


    print '''left x:%d right x:%d  scan dimension: %d''' % (left_x, right_x, scan_dim)

    claim_rect = cvRect(left_x,both_y,scan_dim,scan_dim)
    
    left_worker_thread = threading.Thread(target=worker.detect,args=(claim_rect,zone1))
