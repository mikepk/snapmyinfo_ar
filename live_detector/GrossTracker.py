...
class GrossTracker():
    ...
    def detect(self, window_name):
        ...
        frame = cvCreateImage( orig_size, 8, 3)
        small_frame = cvCreateImage( small_size, 8, 3)
        ...
        while self.running:
            ...
            if squares:
                ...
                #     gfxdraw.rectangle(self.paint_buffer, draw_rect, Color("red"))
                    ...
                    # # if p_mat:
                    # #     cvWarpPerspective(small,frame,p_mat)
                    # 
                    # small = cvGetSubRect(frame, None, bounds)
            ...
            # #     cvPyrDown( small_frame, small_image, CV_GAUSSIAN_5x5 );
            # #     small_frame = small_image
            # #     scale = 2.0

            # frame = cvGetSubRect(frame, None, rect)
            ...
            # frame = cvCreateImage(cvSize(int(new_width),int(self.frame_buffer.height * scale)),8,3)
            # cvResize(self.frame_buffer,frame)
...
def main():
    ...
    worker.small_frame = cvCreateImage((int(o_width / 4.0), int(o_height / 4.0)), 8, 3)
    frame = cvCreateImage((int(o_width),int(o_height)), 8, 3)
    ...
    while running:
        ...
        # cvCopy(worker.frame_buffer, frame) #CloneImage(worker.frame_buffer)
        
        # if resize:
        #     cvResize(frame,resiz)
        ...
        surf_dat = worker.frame_buffer.as_numpy_array().transpose(1,0,2)[...,...,::-1] #,0,2) # ipl_to_numpy(disp_img
